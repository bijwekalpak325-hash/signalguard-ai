...
function getSignal(phase: Phase) {
  if (phase === "REQUESTED" || phase === "RESTORED") return "YELLOW";
...
export default function DigitalTwinPage() {
  ...
  const [selectedTrackId, setSelectedTrackId] = useState<string | number | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    const exists = tracks.some(
      (track) =>
        (track.trackId ?? track.id) === selectedTrackId,
    );
